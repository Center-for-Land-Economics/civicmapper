...
- A city card in [cities.html](../viz/cities.html)
...
Use PMTiles when any of these hold:

- **parcel count is large — rule of thumb ~100k+ parcels** (when in doubt, bake it)
- browser parsing is slow
- the full dataset makes the map unstable
- low-zoom rendering needs a pre-aggregated layer

Examples already using PMTiles: Denver, NYC, Baltimore, Houston, Austin, Dallas,
San Antonio, Detroit.

### H3 hex aggregation is the standard low-zoom layer

**Every PMTiles city ships an H3 hexagon low-zoom aggregate.** It is the default —
`parquet_to_pmtiles.py` builds the H3 `parcels_low` layer unless you explicitly pass
`--no-h3`. H3 hexes (rates area-weighted, totals summed) render cleanly when zoomed out
and avoid the artifacts of the legacy square-grid aggregate; `--no-h3` exists only to
reproduce old bakes. **Do not add a new PMTiles city without H3.**

Generate PMTiles with (H3 is on by default):

```bash
python data/scripts/parquet_to_pmtiles.py \
  --city <city> \
  --file data/jurisidictions/data/<city>/<city>-<state>-parcels.parquet \
  --drop-remnants \   # if the city sets hideRemnants:true (keeps hexes consistent)
  --wsl \             # on Windows (tippecanoe + pmtiles live in WSL)
  --upload --overwrite
```
...
- builds the H3 low-zoom aggregate layer (`parcels_low`)
- builds `parcels_under` — the underutilized subset (Vacant / Parking Lot / Underdeveloped)
  tiled across ALL zooms, so the "Vacant & Underdeveloped" tab renders when zoomed out
  (the full parcel layer can't tile low-zoom; the subset is small enough that it can)
- creates `.pmtiles` (H3 hexes z0–12 + full parcels z13–14 + `parcels_under` z2–14, tile-joined)
- uploads PMTiles + metadata to blob storage

Verify the bake contains all three layers before uploading:

```bash
pmtiles show --metadata <file>.pmtiles   # expect vector_layers:
#   parcels (z13-14), parcels_low (z0-12), parcels_under (z2-14)
# NOTE: for most cities the bake prunes the r12 hex band (finer than the median parcel)
# and hands off early: parcels (z12-14), parcels_low (z0-11). The bake log prints the
# decided ladder, and the metadata JSON's `parcelMinZoom` (which the frontend prefers
# over the dictionary's static 13) must match. See docs/pmtiles-h3-hexagons.md.
```

The metadata JSON gets `"underutilizedSourceLayer": "parcels_under"`, which the frontend
reads to point the underutilized tab at the all-zoom subset (older bakes lacking the key
fall back to the z13+ `parcels` layer, so re-bake to give that tab low-zoom rendering).

If a city looks bad fully zoomed out, tune the H3 zoom bands / resolutions in:

- [parquet_to_pmtiles.py](../data/scripts/parquet_to_pmtiles.py) (`H3_RESOLUTIONS`, `H3_ZOOM_BANDS`)
- city dictionary `_config` (`lowZoomFadeStart`/`lowZoomFadeEnd`/`parcelMinZoom`)
...
- full NAIP + ML segmentation pipeline (recommended — OSM-only badly undercounts parking
  outside well-mapped downtowns). Default-on quality steps: NIR vegetation strip, OSM road
  removal, and a parcel-context false-positive flag. Needs the `data/.venv` GPU env; see
  `data/scripts/parking_lot_extraction.py` and `data/scripts/cloud_bootstrap.sh` for setup
  (including the Windows NAIP-fetch DNS workaround baked into the fetch path).
- `--osm-only` fast fallback (what all shipped cities currently use).
...
# disable a post-processing step if needed: --no-nir-strip / --no-remove-roads
```
...
See any existing file in [viz/src/cities/](../viz/src/cities/) for the shape;
the full field reference is the `CityDef` interface in
[cities.ts](../viz/src/cities.ts).
...
- `VITE_API_BASE` / `VITE_TILE_API_URL`: the dev deploy workflow injects the
  dev API/tile endpoints (see `.github/workflows/deploy-frontend-dev.yml`)
...
After upload, validate on the dev deployment:

- `/app.html?city=<city>`
- `/parking.html?city=<city>` if parking exists
...
- City card added to `cities.html`
